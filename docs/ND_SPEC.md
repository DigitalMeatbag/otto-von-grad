...
- `TgSelfAttention`, `TgBlock`, `TgTransformer`, `TgGPT` architecture modules — these carry forward in function but require significant internal rewrites. The op migration is a prerequisite; no module can be migrated until all ops it depends on support N-D shapes. Key decisions recorded here:
  ...
  **`TgBlock` LayerNorm parameters** (`gamma`, `beta`) are `[1, C]` and are passed directly to `tg_layer_norm` — no expansion required or permitted. `tg_layer_norm` normalizes over the last axis for any N-D input (e.g., `a` is `[B, T, C]`); it requires exactly `C` elements in `gamma`/`beta`, so the `[1, C]` form is correct as-is. FFN biases (`B1`, `B2`) are `[1, ffn_dim]`/`[1, C]` and still require explicit expansion to `[B, T, ffn_dim]`/`[B, T, C]` for `tg_add`, which enforces exact shape match — see §4.2 for the `tg_reshape` + `tg_expand_dim` sequence.
