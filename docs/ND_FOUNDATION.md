...
- All ops in `tg_ops.h` (elementwise, matmul, attention, norm, loss, etc.), with the following exceptions and changes:
  ...
  - **Generalized:** `tg_transpose(a)` → `tg_transpose(a, dim0, dim1)` (swap any two axes; old one-argument form is a hard cut, no shim). `tg_softmax_rows(a)` → `tg_softmax(a, axis)` (softmax along a specified axis). `tg_causal_mask(a)` — signature unchanged; implementation generalizes from 2D `[T, T]` to N-D, masking the last two dimensions uniformly across all leading dims. `tg_layer_norm` normalizes over `ndim-1` (the feature/channel axis) by design — no `axis` parameter. LayerNorm is defined over the feature dimension and the axis does not vary at call sites; an `axis` parameter would be misleading API surface with no real use case.
