...
### 4.1 — N-D Tensor: Option A **[RESOLVED]**
...
- `rows` and `cols` are retained as computed aliases (`shape[ndim-2]` and `shape[ndim-1]`) so existing op code continues to compile during migration. They are removed in the same change that migrates the last op — no separate cleanup commit, no deprecated wrappers.
...
### 5.4 — API Break Strategy **[RESOLVED]**
...
### 5.5 — TG_MAX_GRAPH and TG_MAX_PARENTS **[RESOLVED]**
...
**1. Explicit over implicit.** No silent broadcasting. No inferred shapes. If a shape is unexpected, fail loudly. This is non-negotiable — it is what makes a hand-written autograd engine debuggable.
...
- All ops in `tg_ops.h` (elementwise, matmul, attention, norm, loss, etc.), with the following exceptions and changes:
  - **Removed:** `tg_repeat_rows`, `tg_repeat_cols`, `tg_slice_cols`, `tg_concat_cols`, `tg_concat_rows`. These are 2D-specific with no N-D use case. Hard cut; no shims.
  - **Generalized:** `tg_transpose(a)` → `tg_transpose(a, dim0, dim1)` (swap any two axes). `tg_softmax_rows(a)` → `tg_softmax(a, axis)` (softmax along a specified axis). `tg_causal_mask(a)` — signature unchanged; implementation generalizes from 2D `[T, T]` to N-D, masking the last two dimensions uniformly across all leading dims.
